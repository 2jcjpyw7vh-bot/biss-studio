...
const aliases={"קטלבל":"קטלבל 4–8 ק״ג","גומיות ירכיים":"גומיות ירכיים","גומיות התנגדות":"גומיות התנגדות","גומיות עם ידיות":"גומיות עם ידיות","כדורי פיזיו":"כדורי פיזיו","גלילי פילאטיס":"גלילי פילאטיס","מזרני פילאטיס":"מזרני פילאטיס","מדרגה":"מדרגה","בוסו":"בוסו","פיתה שיווי משקל":"פיתה שיווי משקל","קוביות פילאטיס":"קוביות פילאטיס","כדורי משקולת 1 ק״ג":"כדורי משקולת 1 ק״ג","אוברבול":"אוברבול","בר":"בר קבוע בסטודיו","נקודת עגינה רצפתית":"נקודת עיגון לגומיות - רצפה","נקודת עגינה גובה מותן":"נקודת עיגון לגומיות - גובה מותן"};   if(label==="משקולות יד")return inventory.filter(x=>x.category==="dumbbells"&&x.active!==false).reduce((s,x)=>s+x.quantity,0);
...
async function addEquipment(e){
 ...
 return <main className="shell v26-shell">
  ...
  {tab==="builder"&&<section className="panel hero-panel v26-builder-panel"><h2>בניית אימון</h2><div className="sub">בחרו מטרה בסיסית. המערכת מתחשבת במלאי ומסמנת מתי נדרשת עבודה בזוגות או רוטציה.</div><div className="filters"><label className="field">אזור גוף<select value={region} onChange={e=>setRegion(e.target.value)}>{allRegions.map(x=><option key={x}>{x}</option>)}</select></label><label className="field">ציוד<select value={equipment} onChange={e=>setEquipment(e.target.value)}>{allEquipment.map(x=><option key={x}>{x}</option>)}</select></label><label className="field">מבנה<select value={structure} onChange={e=>setStructure(e.target.value)}>{structures.map(x=><option key={x}>{x}</option>)}</select></label><label className="field">רמה<select value={level} onChange={e=>setLevel(e.target.value)}>{levels.map(x=><option key={x}>{x}</option>)}</select></label><label className="field">משתתפות<input type="number" min="1" max="40" value={people} onChange={e=>setPeople(+e.target.value)}/></label><label className="field">משך בדקות<input type="number" min="20" max="90" value={minutes} onChange={e=>setMinutes(+e.target.value)}/></label></div><div className="safe" style={{marginTop:14}}>

...
export default function Home(){
 ...
 useEffect(()=>{
   ...
   if(["builder","exercises","homework","program","equipment"].includes(requested)) setTab(requested);
 },[]);
 ...
 return <main className="shell v26-shell">
  ...
  <nav className="tabs v26-tabs v27-tabs" aria-label="ניווט ראשי">{[["builder","בניית אימון"],["exercises","תרגילים"],["homework","שיעורי בית"],["program","תוכנית"],["equipment","ציוד"]].map(([id,label])=><button key={id} className={tab===id?"active":""} onClick={()=>{setTab(id);if(id==="equipment")refreshInventory()}}>{label}</button>)}{workout&&<button className={tab==="workout"?"active":""} onClick={()=>setTab("workout")}>האימון שלי</button>}</nav>

  {tab==="builder"&&<section className="panel hero-panel v26-builder-panel v27-panel v27-builder"><h2>בניית אימון</h2><div className="sub">בחרו מטרה בסיסית. המערכת מתחשבת במלאי ומסמנת מתי נדרשת עבודה בזוגות או רוטציה.</div><div className="filters"><label className="field">אזור גוף<select value={region} onChange={e=>setRegion(e.target.value)}>{allRegions.map(x=><option key={x}>{x}</option>)}</select></label><label className="field">ציוד<select value={equipment} onChange={e=>setEquipment(e.target.value)}>{allEquipment.map(x=><option key={x}>{x}</option>)}</select></label><label className="field">מבנה<select value={structure} onChange={e=>setStructure(e.target.value)}>{structures.map(x=><option key={x}>{x}</option>)}</select></label><label className="field">רמה<select value={level} onChange={e=>setLevel(e.target.value)}>{levels.map(x=><option key={x}>{x}</option>)}</select></label><label className="field">משתתפות<input type="number" min="1" max="40" value={people} onChange={e=>setPeople(+e.target.value)}/></label><label className="field">משך בדקות<input type="number" min="20" max="90" value={minutes} onChange={e=>setMinutes(+e.target.value)}/></label></div><div className="safe" style={{marginTop:14}}>
